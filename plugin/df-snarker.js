...
function expectedRadius(currTime = Date.now() / 1000) {
  if(df.contractConstants.SHRINK > 0) {
    return shrinkAlgorithm(currTime, df.contractConstants);
  }
  else {
...
async function snark(actionId, oldX, oldY, newX, newY) {
  ...
  const futureRadius = expectedRadius(Date.now() / 1000 + tenMinutesInSeconds)
  try {
    ...
    const cacheKey = `${oldX}-${oldY}-${newX}-${newY}-${distMax}`;
...
class Plugin {
  constructor() {
    ...
    df.move = move;
    this.addSnarker("http://0.0.0.0:8082/move", 4);
  }
